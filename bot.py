...
# Текст с информацией о менеджере
MANAGER_INFO = "\n\n📞 Подробности уточняйте у менеджера - @Lcdstock34"
...
async def start(message: types.Message):
    ...
    await message.answer(
        "Выберите категорию:" + MANAGER_INFO,
...
async def forward_from_channel(callback: types.CallbackQuery):
    ...
    if message_id == 0:
        await callback.message.edit_text(
            "🛠 **Информация появится позже**" + MANAGER_INFO,
    ...
    try:
        # Копируем сообщение из канала и добавляем информацию о менеджере
        ...
        # Отправляем отдельное сообщение с информацией о менеджере
        await callback.message.answer(MANAGER_INFO)
        await callback.answer()
...
async def show_display_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "📱 **Выберите тип дисплея:**" + MANAGER_INFO,
...
async def show_battery_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "🔋 **Выберите тип аккумулятора:**" + MANAGER_INFO,
...
async def show_back_cover_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "📦 **Выберите качество задней крышки:**" + MANAGER_INFO,
...
async def show_case_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "🏠 **Выберите качество корпуса:**" + MANAGER_INFO,
...
async def show_waterproof_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "💧 **Выберите тип проклейки:**" + MANAGER_INFO,
...
async def show_speakers_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "🔊 **Выберите тип динамиков:**" + MANAGER_INFO,
...
async def show_camera_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "📸 **Выберите тип камеры:**" + MANAGER_INFO,
...
async def show_cables_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "🔌 **Выберите тип шлейфа:**" + MANAGER_INFO,
...
async def show_apple_watch_menu(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "⌚ **Выберите запчасть для Apple Watch:**" + MANAGER_INFO,
